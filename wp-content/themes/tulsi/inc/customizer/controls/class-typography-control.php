...
if ( class_exists( 'WP_Customize_Control' ) && ! class_exists( 'Tulsi_Typography_Customize_Control' ) ) {
	/**
	 * Create the typography elements control.
	 *
	 */
	class Tulsi_Typography_Customize_Control extends WP_Customize_Control {
		public $type = 'tulsi-customizer-typography';

		public function enqueue() {
			wp_enqueue_script( 'tulsi-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/selectWoo.min.js', array( 'customize-controls', 'jquery' ), TULSI_VERSION, true );
			wp_enqueue_style( 'tulsi-typography-selectWoo', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/selectWoo.min.css', array(), TULSI_VERSION );

			wp_enqueue_script( 'tulsi-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/js/typography-customizer.js', array( 'customize-controls', 'tulsi-typography-selectWoo' ), TULSI_VERSION, true );
			wp_enqueue_style( 'tulsi-typography-customizer', trailingslashit( get_template_directory_uri() )  . 'inc/customizer/controls/css/typography-customizer.css', array(), TULSI_VERSION );
		}

		public function to_json() {
			parent::to_json();

			$number_of_fonts = apply_filters( 'tulsi_number_of_fonts', 200 );
			$this->json[ 'default_fonts_title'] = __( 'System fonts', 'tulsi' );
			$this->json[ 'google_fonts_title'] = __( 'Google fonts', 'tulsi' );
			$this->json[ 'google_fonts' ] = apply_filters( 'tulsi_typography_customize_list', tulsi_get_all_google_fonts( $number_of_fonts ) );
			$this->json[ 'default_fonts' ] = tulsi_typography_default_fonts();
			$this->json[ 'family_title' ] = esc_html__( 'Font family', 'tulsi' );
			$this->json[ 'weight_title' ] = esc_html__( 'Font weight', 'tulsi' );
			$this->json[ 'transform_title' ] = esc_html__( 'Text transform', 'tulsi' );
			$this->json[ 'category_title' ] = '';
			$this->json[ 'variant_title' ] = esc_html__( 'Variants', 'tulsi' );
		...
		public function content_template() {
			...
			<# if ( 'undefined' !== typeof ( data.variant ) ) { #>
				...
				#>
				<div id={{{ data.variant.id }}}" class="tulsi-font-variant" data-saved-value="{{ data.variant.value }}">
					<label>
			...
			<# if ( 'undefined' !== typeof ( data.category ) ) { #>
				<div class="tulsi-font-category">
					<label>
							<input name="{{{ data.category.id }}}" type="hidden" {{{ data.category.link }}} value="{{{ data.category.value }}}" class="tulsi-hidden-input" />
			...
			<# if ( 'undefined' !== typeof ( data.transform ) ) { #>
				<div class="tulsi-font-transform">
					<label>
