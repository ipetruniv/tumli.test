( function( api ) {
	...
	api.controlConstructor['tulsi-customizer-typography'] = api.Control.extend( {
		ready: function() {
			var control = this;

			control.container.on( 'change', '.tulsi-font-family select',
				function() {
					...
					setTimeout( function() {
						// Send our request to the tulsi_get_all_google_fonts_ajax function
						var response = jQuery.getJSON({
							type: 'POST',
							url: ajaxurl,
							data: {
								action: 'tulsi_get_all_google_fonts_ajax',
								tulsi_customize_nonce: tulsi_customize.nonce
							},
							async: false,
							dataType: 'json',
						});
						...
						if ( id in fonts ) {
							...
							jQuery( '.tulsi-font-family select' ).not( _this ).each( function( key, select ) {
								var parent = jQuery( this ).closest( '.tulsi-font-family' );

								if ( _value == jQuery( select ).val() && _this.data( 'category' ) !== jQuery( select ).data( 'category' ) ) {
									if ( ! got_variants ) {
										updated_variants = jQuery( parent.next( '.tulsi-font-variant' ).find( 'select' ) ).val();
										got_variants = true;
									}
								}
							} );
							...
							_this.closest( '.tulsi-font-family' ).next( 'div' ).show();
							...
							_this.closest( '.tulsi-font-family' ).next( 'div' ).hide();
			...
			control.container.on( 'change', '.tulsi-font-variant select',
				function() {
					var _this = jQuery( this );
					...
					jQuery( '.tulsi-font-variant select' ).each( function( key, value ) {
						var this_control = jQuery( this ).closest( 'li' ).attr( 'id' ).replace( 'customize-control-', '' );
						var parent = jQuery( this ).closest( '.tulsi-font-variant' );
			...
			control.container.on( 'change', '.tulsi-font-category input',
				function() {
					control.settings['category'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.tulsi-font-weight select',
				function() {
					control.settings['weight'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.tulsi-font-transform select',
				function() {
...
jQuery( document ).ready( function($) {
	...
	jQuery( '.tulsi-font-family select' ).selectWoo();
	jQuery( '.tulsi-font-variant' ).each( function( key, value ) {
		var _this = $( this );
	...
	$( ".tulsi-font-family" ).each( function( key, value ) {
		var _this = $( this );
		if ( $.inArray( _this.find( 'select' ).val(), typography_defaults ) !== -1 ) {
			_this.next( '.tulsi-font-variant' ).hide();
		}
	});
