...
add_action( 'after_setup_theme', 'tulsi_setup_woocommerce' );
/**
 * Set up WooCommerce
 *
 */
function tulsi_setup_woocommerce() {
	...
	add_action( 'woocommerce_sidebar', 'tulsi_construct_sidebars' );
}

if ( ! function_exists( 'tulsi_woocommerce_start' ) ) {
	add_action( 'woocommerce_before_main_content', 'tulsi_woocommerce_start', 10 );
	/**
	 * Add WooCommerce starting wrappers
	 *
	 */
	function tulsi_woocommerce_start() { ?>
...
if ( ! function_exists( 'tulsi_woocommerce_end' ) ) {
	add_action( 'woocommerce_after_main_content', 'tulsi_woocommerce_end', 10 );
	/**
	 * Add WooCommerce ending wrappers
	 *
	 */
	function tulsi_woocommerce_end() { ?>
						</div><!-- .entry-content -->
						<?php
						/**
...
if ( ! function_exists( 'tulsi_woocommerce_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'tulsi_woocommerce_css', 100 );
	/**
	 * Add WooCommerce CSS
	 *
	 */
	function tulsi_woocommerce_css() {
		if ( ! class_exists( 'WooCommerce' ) ) {
			return;
		}

		$mobile = apply_filters( 'tulsi_mobile_media_query', '(max-width:768px)' );
...
if ( ! function_exists( 'tulsi_bbpress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'tulsi_bbpress_css', 100 );
	/**
	 * Add bbPress CSS
	 *
	 */
	function tulsi_bbpress_css() {
...
if ( ! function_exists( 'tulsi_buddypress_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'tulsi_buddypress_css', 100 );
	/**
	 * Add BuddyPress CSS
	 *
	 */
	function tulsi_buddypress_css() {
...
if ( ! function_exists( 'tulsi_beaver_builder_css' ) ) {
	add_action( 'wp_enqueue_scripts', 'tulsi_beaver_builder_css', 100 );
	/**
	 ...
	 *
	 */
	function tulsi_beaver_builder_css() {
