...
if ( ! function_exists( 'tulsi_enqueue_google_fonts' ) ) {
	add_action( 'wp_enqueue_scripts', 'tulsi_enqueue_google_fonts', 0 );
	/**
	 * Add Google Fonts to wp_head if needed.
	 *
	 */
	function tulsi_enqueue_google_fonts() {
		...
		$not_google = str_replace( ' ', '+', tulsi_typography_default_fonts() );
		...
		if ( ! empty( $font_settings ) ) {
			...
			foreach ( $font_settings as $key ) {
				...
				if ( ! isset( $tulsi_settings[$key] ) ) {
					continue;
				}

				// If our value is still using the old format, fix it
				if ( strpos( $tulsi_settings[$key], ':' ) !== false ) {
					$tulsi_settings[$key] = current( explode( ':', $tulsi_settings[$key] ) );
				}

				// Replace the spaces in the names with a plus
				$value = str_replace( ' ', '+', $tulsi_settings[$key] );

				// Grab the variants using the plain name
				$variants = tulsi_get_google_font_variants( $tulsi_settings[$key], $key );
		...
		$google_fonts = apply_filters( 'tulsi_typography_google_fonts', $google_fonts );

		// Get the subset
		$subset = apply_filters( 'tulsi_fonts_subset','' );
		...
		$default_fonts = TULSI_DEFAULT_FONTS;

		// Create our URL using the arguments
		$fonts_url = add_query_arg( $font_args, '//fonts.googleapis.com/css' );

		// Enqueue our fonts
		wp_enqueue_style('tulsi-default-fonts', $default_fonts, array(), null, 'all' );
		if ( $google_fonts ) {
			wp_enqueue_style('tulsi-fonts', $fonts_url, array(), null, 'all' );
		}
	}
}

if ( ! function_exists( 'tulsi_default_fonts_customize_register' ) ) {
	add_action( 'customize_register', 'tulsi_default_fonts_customize_register' );
	/**
	 * Build our Typography options
	 *
	 */
	function tulsi_default_fonts_customize_register( $wp_customize ) {
		if ( function_exists( 'tulsi_fonts_customize_register' ) ) {
			// Bail if Tulsi Premium is active
			return;
		}
		...
		$defaults = tulsi_get_default_fonts();

		if ( method_exists( $wp_customize,'register_control_type' ) ) {
			$wp_customize->register_control_type( 'Tulsi_Typography_Customize_Control' );
		...
		$wp_customize->add_setting(
			...
			array(
				...
				'sanitize_callback' => 'tulsi_sanitize_variants'
			)
		);

		$wp_customize->add_setting(
			'tulsi_settings[body_font_weight]',
			array(
		...
		$wp_customize->add_setting(
			'tulsi_settings[body_font_transform]',
			array(
		...
		$wp_customize->add_control(
			new Tulsi_Typography_Customize_Control(
				$wp_customize,
				'body_typography',
				array(
					'section' => 'font_section',
					'priority' => 1,
					'label' => __( 'Text Font', 'tulsi' ),
					'settings' => array(
						'family' => 'tulsi_settings[font_body]',
						'variant' => 'font_body_variants',
						'category' => 'font_body_category',
						'weight' => 'tulsi_settings[body_font_weight]',
						'transform' => 'tulsi_settings[body_font_transform]',
					),
				)
			)
		);
		...
		$wp_customize->add_control(
			new Tulsi_Range_Slider_Control(
				...
				array(
					...
					'description' => __( 'Font size', 'tulsi' ),
					'section' => 'font_section',
					'settings' => array(
						'desktop' => 'tulsi_settings[body_font_size]',
					),
					'choices' => array(
						'desktop' => array(
							'min' => 6,
		...
		$wp_customize->add_setting(
			'tulsi_settings[body_line_height]',
			array(
				'default' => $defaults['body_line_height'],
				'type' => 'option',
				'sanitize_callback' => 'tulsi_sanitize_decimal_integer',
				'transport' => 'postMessage'
			)
		);
		...
		$wp_customize->add_control(
			new Tulsi_Range_Slider_Control(
				...
				array(
					'type' => 'tulsi-range-slider',
					'description' => __( 'Line height', 'tulsi' ),
					'section' => 'font_section',
					'settings' => array(
						'desktop' => 'tulsi_settings[body_line_height]',
					),
					'choices' => array(
						'desktop' => array(
							'min' => 1,
		...
		$wp_customize->add_setting(
			'tulsi_settings[paragraph_margin]',
			array(
		...
		$wp_customize->add_control(
			new Tulsi_Range_Slider_Control(
				...
				array(
					'type' => 'tulsi-range-slider',
					'description' => __( 'Paragraph margin', 'tulsi' ),
					'section' => 'font_section',
					'settings' => array(
						'desktop' => 'tulsi_settings[paragraph_margin]',
					),
					'choices' => array(
						'desktop' => array(
							'min' => 0,
...
if ( ! function_exists( 'tulsi_get_all_google_fonts' ) ) {
	/**
	 ...
	 */
	function tulsi_get_all_google_fonts( $amount = 'all' ) {
		...
		if ( apply_filters( 'tulsi_alphabetize_google_fonts', true ) ) {
			asort( $fonts );
		}

		// Filter to allow us to modify the fonts array
		return apply_filters( 'tulsi_google_fonts_array', $fonts );
	}
}

if ( ! function_exists( 'tulsi_get_all_google_fonts_ajax' ) ) {
	add_action( 'wp_ajax_tulsi_get_all_google_fonts_ajax', 'tulsi_get_all_google_fonts_ajax' );
	/**
	 * Return an array of all of our Google Fonts.
	 *
	 *
	 */
	function tulsi_get_all_google_fonts_ajax() {
		// Bail if the nonce doesn't check out
		if ( ! isset( $_POST[ 'tulsi_customize_nonce' ] ) || ! wp_verify_nonce( sanitize_key( $_POST[ 'tulsi_customize_nonce' ] ), 'tulsi_customize_nonce' ) ) {
			wp_die();
		}

		// Do another nonce check
		check_ajax_referer( 'tulsi_customize_nonce', 'tulsi_customize_nonce' );
		...
		$fonts = tulsi_get_all_google_fonts();
...
if ( ! function_exists( 'tulsi_get_google_font_variants' ) ) {
	/**
	 * Wrapper function to find variants for chosen Google Fonts
	 * Example: tulsi_get_google_font_variation( 'Open Sans' )
	 *
	 */
	function tulsi_get_google_font_variants( $font, $key = '' ) {
		// Don't need variants if we're using a system font
		if ( in_array( $font, tulsi_typography_default_fonts() ) ) {
			return;
		}
		...
		if ( $variants ) {
			...
			return implode(',', apply_filters( 'tulsi_typography_variants', $output ) );
		}
	}
}

if ( ! function_exists( 'tulsi_get_google_font_category' ) ) {
	/**
	 * Wrapper function to find the category for chosen Google Font
	 * Example: tulsi_get_google_font_category( 'Open Sans' )
	 *
	 *
	 *
	 ...
	 */
	function tulsi_get_google_font_category( $font, $key = '' ) {
...
if ( ! function_exists( 'tulsi_get_font_family_css' ) ) {
	/**
	 ...
	 */
	function tulsi_get_font_family_css( $font, $settings, $default ) {
		$tulsi_settings = wp_parse_args(
		...
		$no_quotes = array(
			...
			apply_filters( 'tulsi_typography_system_stack', '-apple-system, system-ui, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol"' )
		);

		// Get our font
		$font_family = $tulsi_settings[ $font ];

		if ( 'System Stack' == $font_family ) {
			$font_family = apply_filters( 'tulsi_typography_system_stack', '-apple-system, system-ui, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol"' );
		}
		...
		if ( in_array( $font_family, $no_quotes ) ) {
			...
			$wrapper_start = '"';
			$wrapper_end = '"' . tulsi_get_google_font_category( $font_family, $font );
		}
...
if ( ! function_exists( 'tulsi_add_to_font_customizer_list' ) ) {
	add_filter( 'tulsi_typography_customize_list', 'tulsi_add_to_font_customizer_list' );
	/**
	 ...
	 *
	 */
	function tulsi_add_to_font_customizer_list( $fonts ) {
		...
		$all_fonts = tulsi_get_all_google_fonts();
		$select_fonts = tulsi_get_all_google_fonts( apply_filters( 'tulsi_number_of_fonts', 200 ) );

		foreach ( $font_settings as $setting ) {
			// If we don't have a setting, keep going
			if ( ! isset( $tulsi_settings[ $setting ] ) ) {
				continue;
			}

			$id = strtolower( str_replace( ' ', '_', $tulsi_settings[ $setting ] ) );

			if ( array_key_exists( $id, $select_fonts ) || in_array( $tulsi_settings[ $setting ], tulsi_typography_default_fonts() ) ) {
				continue;
			}

			$fonts[ strtolower( str_replace( ' ', '_', $tulsi_settings[ $setting ] ) ) ] = array(
				'name' => $tulsi_settings[ $setting ],
		...
		if ( function_exists( 'tulsi_secondary_nav_get_defaults' ) ) {
			...
			if ( ! array_key_exists( $secondary_nav_id, $select_fonts ) && ! in_array( $secondary_nav_settings[ 'font_secondary_navigation' ], tulsi_typography_default_fonts() ) ) {
